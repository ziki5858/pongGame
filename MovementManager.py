...
class MovementManager:
    ...
    @staticmethod
    def _move_player_if_key(event, sprite, key_map):
        x, y = sprite.get_pos()
    ...
    @staticmethod
    def handle_ai_move(rSprite, ball_center):
        xR, yR = rSprite.get_pos()
        paddle_center = yR + rSprite.get_height() * 0.5
        delta = ball_center - paddle_center
        if abs(delta) <= MovementManager.AI_DEAD_ZONE:
            step = 0
        else:
            step = max(-MovementManager.sprite_speed, min(delta, MovementManager.sprite_speed))
    ...
    def sprite_movement(event):
        ...
        if event:
            MovementManager._move_player_if_key(event, lSprite, MovementManager.UP_DOWN_LEFT)
            if not GlobalData.against_com:
                MovementManager._move_player_if_key(event, rSprite, MovementManager.UP_DOWN_RIGHT)

        if GlobalData.against_com:
            MovementManager.handle_ai_move(rSprite, ball_center)
