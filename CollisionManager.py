...
class CollisionManager:
    ...
    def handle_paddle_collision(ball, left_paddle, right_paddle,
        ...
        for paddle, side in ((right_paddle, 'r'), (left_paddle, 'l')):
            if CollisionManager._check_overlap(
                ...
                new_x = direction * abs(x_move) + direction * BallSpeedPix
    ...
    def handle_side_collision(ball, index, x_ball, sound_list):
        ...
        if len(paddles) == 2:
            ...
            if x_ball <= 0:
                left_paddle.lose_life('left')
            else:
                right_paddle.lose_life('right')
    ...
    def check_game_over():
        ...
        if left_life <= 0:
            ...
            CollisionManager.wait_for_restart()
        elif right_life <= 0:
