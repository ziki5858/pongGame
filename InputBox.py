...
class InputBox:
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            self.error = not (self.text.isdigit() and
                              self.valid_range[0] <= int(self.text) <= self.valid_range[1])
    ...
    def draw(self, screen):
        ...
        border_color = (self.error_color if self.error else
                        self.active_color if self.active else pygame.Color('gray'))
...
def game_settings():
    ...
    while running:
        ...
        # draw button
        GameTextManager.draw_button(btn_rect, "Let's Play!", font_size, valid)

        # apply settings if confirmed
